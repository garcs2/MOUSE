...
#                                                Sec. 1: Materials
# **************************************************************************************************************************
for params['XS_type'] in ['endf8.1', 'endf8.0']:
    if params['XS_type'] == 'endf8.0':       # string == string ✓
        update_params({
            'cross_sections_xml_location': '/hpc-common/data/openmc/endfb-viii.0-hdf5/cross_sections.xml'
        })
    elif params['XS_type'] == 'endf8.1':     # string == string ✓
        update_params({
    ...
    update_params({
        ...
        'U_met_wo': 0.3, 
        'er_wo': 0,    # Weight ratio of Uranium to total fuel weight (less than 1)
        'Coolant': 'NaK',
    ...
    build_openmc_model = build_openmc_model_LTMR_3D
    ...
    update_params({
        ...
        'Vessel Lower Plenum Height': 50,  # cm, based on Reflecting Barrel~RPV Liner (-Reflector Thickness, which is currently missing in CAD),  # cm
    ...
    params['Onsite Coolant Inventory'] = 1833 * params['Power MWt'] # kg
